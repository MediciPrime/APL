...
# Provide input data
die "usage:$0 <Sample> <Sample.classify_uc.txt> <percentage_threshold>\n" unless $#ARGV == 2;
my $start_run = time();
...
open(my $fh_out, ">", $sample.".taxonomy_table_Reads_Mar14_17.csv");
...
for my $clade (@classification){
	...
	for my $entry (@sorted_entries){
		...
		writePercentage($percentage, $entry, $num_of_Reads, $clade, \%parent_child);
	}
}
...
sub writePercentage{
	...
	if ($percentage > $percentage_threshold){
		if ($entry ne ""){
			print $fh_out "$entry,$num_of_Reads," . $percentage * 100 . "%," . $upper_number . "%, $parent\n";
		}
	}
}
