...
# Common usage: calcTaxonomy_byReads.pl Sample Sample.classify_uc.txt 0.01

# Provide input data
die "usage:$0 <Sample.classify_uc.txt> <percentage_threshold>\n" unless $#ARGV == 2;
my $start_run = time();

my %taxonomy_tree; # hash of hash. key1 = clade, key2 = entry, value = number of reads
my %parent_child; # stores the "parent child" relationship where the key is the child and the value is the parent
...
while (<$fh>){
	...
	for my $clade (@classification) {
		if (defined( my $index = $indices{$clade} )) {
			my $entry = @cells[$index - 1];
			my $confidence = @cells[$index + 1];
...
sub get_child{
	...
	my $entry_child = "";
	@indices{ @cells } = 0 .. $#cells;
	if (defined( my $index = $indices{ $taxonomic_hierarchy{$clade} } )) {
		$entry_child = @cells[$index - 1];
	}
...
open(my $fh_out, ">", $sample.".taxonomy_table_Reads.csv");
print $fh_out "Total number of Reads,$taxonomy_tree{'total_number_of_roots'}\n";
...
for my $clade (@classification){
	...
	if ($clade eq "domain"){
		print $fh_out "Rank: Domain,Number of Reads, Percentage of Domain,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "phylum"){
		print $fh_out "\nRank: Phylum,Number of Reads, Percentage of Phylum,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "class"){
		print $fh_out "\nRank: Class,Number of Reads, Percentage of Class,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "order"){
		print $fh_out "\nRank: Order,Number of Reads, Percentage of Order,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "family"){
		print $fh_out "\nRank: Family,Number of Reads, Percentage of Family,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "genus"){
		print $fh_out "\nRank: Genus,Number of Reads, Percentage of Genus,Percent of Prior Rank, Prior Rank\n";
	}
	...
	for my $entry (@sorted_entries){
		...
		if ($clade eq "domain"){
			...
			$num_of_Reads = $taxonomy_tree{$clade}{$entry};
			$percentage = $num_of_Reads/$taxonomy_tree{'total_number_of_roots'};
			writePercentage($percentage, $entry, $num_of_Reads, $clade, \%parent_child, \%pecentConfidence);
...
sub writePercentage{
	...
	if ($entry eq "Bacteria" || $entry eq "Archaea") {
		$upper_number = $num_of_Reads / $taxonomy_tree{'total_number_of_roots'} * 100;
		$parent = "root";
	} else {
		...
		if ($clade eq "genus" and $taxonomy_tree{$parent_clade_map{$parent}}{$genus_parent_child{$entry}}){ # and $parent_child{$entry} eq ""){
			$upper_number = $num_of_Reads / $taxonomy_tree{$parent_clade_map{$parent}}{$genus_parent_child{$entry}} * 100;
		} else {
			if ($taxonomy_tree{$clade_parent_child_map{$clade}}{$parent_child{$clade_parent_child_map{$clade}}{$entry}} == 0){
				if ($taxonomy_tree{$parent_clade_map{$entry}}{$parent_child{$clade_parent_child_map{$clade}}{$entry}}){
					$upper_number = $num_of_Reads/$taxonomy_tree{$parent_clade_map{$entry}}{$parent_child{$clade_parent_child_map{$clade}}{$entry}} * 100;
				}
			} else{

				$upper_number = $num_of_Reads/$taxonomy_tree{$clade_parent_child_map{$clade}}{$parent_child{$clade_parent_child_map{$clade}}{$entry}} * 100;

				if ($upper_number == 0) {
					$upper_number = $num_of_Reads/$taxonomy_tree{'total_number_of_roots'};
					$parent = "root";
				}
			}
		}
	...
	if ($percentage > $percentage_threshold){
		if ($entry ne ""){
			print $fh_out "$entry,$num_of_Reads,$average_confidence %," . $percentage * 100 . "%," . $upper_number . "%, $parent\n";
		}
	}
}
...
# script takes 373 seconds. ~ 6-7 minutes