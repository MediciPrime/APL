...
# Provide input data
die "usage:$0 <Sample> <Sample.classify.txt> <percentage_threshold>\n" unless $#ARGV == 2;
my $start_run = time();
...
open(my $fh_out, ">", $sample.".taxonomy_table_OTUs_test.csv");
...
for my $clade (@classification){
	...
	if ($clade eq "domain"){
		print $fh_out "Rank: Domain,Number of OTUs, Percentage of Domain,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "phylum"){
		print $fh_out "\nRank: Phylum,Number of OTUs, Percentage of Phylum,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "class"){
		print $fh_out "\nRank: Class,Number of OTUs, Percentage of Class,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "order"){
		print $fh_out "\nRank: Order,Number of OTUs, Percentage of Order,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "family"){
		print $fh_out "\nRank: Family,Number of OTUs, Percentage of Family,Percent of Prior Rank, Prior Rank\n";
	} elsif ($clade eq "genus"){
		print $fh_out "\nRank: Genus,Number of OTUs, Percentage of Genus,Percent of Prior Rank, Prior Rank\n";
	}
	...
	for my $entry (@sorted_entries){
		...
		writePercentage($percentage, $entry, $num_of_OTUs, $clade, \%parent_child);
	}
}
...
sub writePercentage{
	...
	if ($percentage > $percentage_threshold){
		if ($entry ne ""){
			print $fh_out "$entry,$num_of_OTUs," . $percentage * 100 . "%," . $upper_number . "%, $parent\n";
		}
	}
}
